<?php
    ...
    foreach( $html->find('tr') as $tr ):
        ...
        foreach( $tr->find('.ev_td_li') as $li ):
            ...
            $event_start = removeWeekDay( $event_start );
            $event = $event_start;


            /**
             * Über Startdatum gruppieren
             *  - Titel hinzufügen
             *  - Verein hinzufügen
             *  - Farbe hinzufügen (aus Vereinskartell)
             *  - Monat einfügen
             *  - Von-Zeit einfügen
             *  - Bis-Zeit einfügen
             * 
            ...
            $vereine[$current_color][$event]['title'] = strip_tags( $event_link->title );
            ...
            $vereine[$current_color][$event]['organizer'] = $society_name;
             ...
             * 
             */
            $vereine[$current_color][$event]['color'] = $current_color;
            ...
            $vereine[$current_color][$event]['month'] = seperateMonth( $currentMonth );
            if( count($times) == 1 ){
                // Von Zeit einfügen
                ...
                $event_start_date = strftime("%Y%m%d", $event_start_date); //. "T130000"; // Timestamp formatieren

                $vereine[$current_color][$event]['timefrom'] = $event_start_date;
                $vereine[$current_color][$event]['timeto'] = $event_start_date; // Wird für ics-File benötigt; 
                ...
                $event_end_date = strftime("%Y%m%d", $event_end_date); //. "T130000"; // Timestamp formatieren

                $vereine[$current_color][$event]['timefrom'] = $event_start_date;
                $vereine[$current_color][$event]['timeto'] = $event_end_date;
            }
    ...
    // Array als Sequentiell einrichten
    // $vereine = array_values( $vereine );

    // var_dump($vereine);,

    echo json_encode( $vereine );
